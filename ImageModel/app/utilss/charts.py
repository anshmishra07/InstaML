...
def dimensionality_reduction(df):
    ...
    **Dimensionality reduction helps visualize and understand high-dimensional data:**
    ...
    if method == "PCA":
        ...
        max_components = min(10, numeric_df.shape[1]) if numeric_df.shape[1] > 0 else 2
        min_components = min(2, max_components)
        n_components = st.slider("Number of components to show:", min_components, max_components, min_components)
...
def time_series_analysis(df):
    ...
    **Time series analysis reveals patterns that change over time:**
    ...
    # Rolling statistics
    max_window = min(30, len(df_sorted)//4) if len(df_sorted) > 0 else 3
